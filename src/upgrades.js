...
class Upgrades extends Component {
  ...
  mapeando = (id) => {
    let {verPontos, pegaMult, diminuiPontos, ver} = this.props
    let {check, lista, lista2, cor} = this.state
    if (!check) {
      lista.map(
        (item) => {
          if (item.id === id && verPontos() >= item.preco) {
            pegaMult(item.mult)
            diminuiPontos(item.preco)
            if (item.id === 4) {
              ver()
              this.setState({
                check: true,
      ...
      lista2.map(
        (item) => {
          if (item.id === id && verPontos() >= item.preco) {
            ...
            diminuiPontos(item.preco)
            this.setState({
              lista2: lista2.filter(item => item.id !== id)
            })
          }
        }
      )
